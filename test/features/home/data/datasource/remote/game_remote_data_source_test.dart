...
void main() {
  ...
  setUp(() {
    mockDio = MockDio();
    dataSource = GameRemoteDataSourceImpl(mockDio);
  });

  group('getGames', () {
    /// Success fetched data
    test('should return list of GameModel when the call is successful',
        () async {
      // Arrange
      ...
      when(() => mockDio.get<dynamic>(
          ...
          .thenAnswer((_) async => Response(
              ...
              statusCode: 200,
              requestOptions: RequestOptions(path: 'path/to/resource')));
      ...
      expect(result.getOrElse(() => []), expectedResult);
    ...
    test('should return a failure when the call is unsuccessful', () async {
      // Arrange
      final failure = DataFailure(code: 404, message: 'Not Found');
      when(() => mockDio.get<dynamic>(
            any(),
            queryParameters: any(named: 'queryParameters'),
            options: any(named: 'options'),
      ...
      expect(result.fold((failure) => failure, (_) => null), failure);
      verify(() => mockDio.get<dynamic>(
            any(),
            queryParameters: any(named: 'queryParameters'),
            options: any(named: 'options'),
    ...
    test('should return a failure when the call is unsuccessful', () async {
      // Arrange
      final failure = DataFailure(code: -1, message: 'Unknown Error');
      when(() => mockDio.get<dynamic>(
        any(),
        queryParameters: any(named: 'queryParameters'),
        options: any(named: 'options'),
